...
class exp_dataset(Data.Dataset):
    ...
    def  __getitem__(self, idx):
        result2 = None
        if self.data[idx][1].item()<0.5:
            result2 = torch.Tensor([1,0])
        else:
            result2 = torch.Tensor([0,1])
        return self.data[idx][0].to("cpu"), result2
...
def select_action_softmax(state,lor):
    ...
    #    result = 0
    pro = (math.exp(output[0][0].item()))/(math.exp(output[0][0].item())+math.exp(output[0][1].item()))
    if random.random() < pro :
        result = 0
    return torch.tensor(result)

#test env
env_soft = gym.make("CartPole-v1",max_episode_steps=250)
num_episodes = 15
reward_list = []
