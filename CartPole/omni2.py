...
def select_action(state,expdata):
    ...
    sample_num = 140
    wd_0 = calculate_w2(torch.concat((torch.cat((state,torch.zeros(1))).unsqueeze(0),torch.FloatTensor(expdata)[400:400+sample_num,:]),0)
                        ,torch.cat((torch.tensor([100]),torch.ones(sample_num)))
                        ,torch.FloatTensor(expdata)
                        ,torch.ones(len(expdata)))
    wd_1 = calculate_w2(torch.concat((torch.cat((state,torch.ones(1))).unsqueeze(0),torch.FloatTensor(expdata)[400:400+sample_num,:]),0) 
                        ,torch.cat((torch.tensor([100]),torch.ones(sample_num)))
                        ,torch.FloatTensor(expdata)
                        ,torch.ones(len(expdata)))
    result = None
