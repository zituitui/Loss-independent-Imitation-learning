...
class ReplayMemory(object):
    def __init__(self, max_size=10000, n_states=4):
        ...
        self.dones_memory = np.zeros(max_size, dtype=np.bool_)
...
if __name__ == "__main__":
    ...
    print('env_para',n_states,n_actions)
    ...
    for episode in range(1, n_episodes+1):
        ...
        for t in range(n_steps):
            ...
            if episode > 1200:
                if random.random():
                    if(dataset is None):
                        dataset = [[state[0],state[1],state[2],state[3],action]]
                    else:
                        dataset.append([state[0],state[1],state[2],state[3],action])
                        #print(dataset[0])
                        #print('@@@@@@',type(dataset))
